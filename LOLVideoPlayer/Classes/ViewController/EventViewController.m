...
#pragma mark - 处理数据
//- (void)handleData{
//    [[CDHttpHelper defaultCDHttpHelper] get:[eventUrlString stringByAppendingFormat:@"1"] params:nil success:^(id responseObj) {
...
- (void)refreshHandleData:(NSInteger)page{
    ...
    [[CDHttpHelper defaultCDHttpHelper] get:[eventUrlString stringByAppendingFormat:@"%ld",self.page] params:nil success:^(id responseObj) {
        
        [FMDBSaveDataHelper FMDBSaveDataHelperWithData:responseObj andName:@"event"];
        ...
        if ([db open]) {
            ...
            FMResultSet *resultSet = [db executeQuery:@"select *from videoPlayer where name = 'event'"];
            id data = nil;
