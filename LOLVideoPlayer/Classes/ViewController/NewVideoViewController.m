...
@interface NewVideoViewController () <UITableViewDataSource,UITableViewDelegate>
...
- (void)refreshHandleData:(NSInteger)page{
    ...
    [[CDHttpHelper defaultCDHttpHelper] get:[NSString stringWithFormat:classifyNavigationDetailUrlString,@"newest",(long)self.page] params:nil success:^(id responseObj) {
        
        [FMDBSaveDataHelper FMDBSaveDataHelperWithData:responseObj andName:@"newVideo"];
        ...
        if ([db open]) {
            ...
            FMResultSet *resultSet = [db executeQuery:@"select *from videoPlayer where name = 'newVideo'"];
            id data = nil;
